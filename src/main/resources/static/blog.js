            ...
            <div class="post-content-area">
                ...
                <div class="post-actions">
                    ...
                    <div class="action-item delete-btn" 
                         style="display: ${(post.authorName === localStorage.getItem('fullName') || localStorage.getItem('userRole') === 'ADMIN') ? 'flex' : 'none'}; 
                                ${localStorage.getItem('userRole') === 'ADMIN' ? 'color: #ff4d4d; border-color: rgba(255,77,77,0.3); font-weight:800;' : ''}" 
                         onclick="deletePost(${post.id})">
                        <span>🗑</span> ${localStorage.getItem('userRole') === 'ADMIN' ? 'MODERATE (DELETE)' : 'Delete'}
                    </div>
...
async function fetchUserProfile(email) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (navUserName) navUserName.innerText = user.fullName || email.split('@')[0];
            const navUserRole = document.getElementById("navUserRole");
            if (navUserRole) navUserRole.innerText = user.role || "USER";
